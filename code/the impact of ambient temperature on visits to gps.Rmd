...
diag_im<- Diag1
diag_im$class<- icd10.to.class(diag_im$icd10)
View(diag_im[as.logical(sus.entry.vec),])
View(data.frame(diag_entry = Diag1$icd10[sus.entry.vec], class = icd10.to.class(Diag1$icd10[sus.entry.vec])))
```
...
chronic.sel<- grepl("DD",Diag1$DiagTyp)
chronic.df<- data.frame(uniPatID = Diag1$uniPatID[chronic.sel], 
                        diag_category = Diag1$icd10[chronic.sel])|>
  distinct()
chronic.df$diag_category<- icd10.to.class(chronic.df$diag_category)
im<- Diag1[!chronic.sel,]|>
  filter(TG_DateNum >= date.range[1] & TG_DateNum <= date.range[2])|>
  distinct()
