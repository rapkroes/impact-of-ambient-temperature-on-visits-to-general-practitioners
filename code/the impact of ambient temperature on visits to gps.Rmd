...
hyperpars_age<- genetic.algorithm(optim.seed = 4567, n = 50, pcrossover = 0.8, 
                                  ...
                                  y = y.age, est_type = "quantile", alpha = 0.5, 
                                  cv = 2L, no_trees = 100L, 
...
hyperpars_gender<- genetic.algorithm(optim.seed = 4567, n = 50, 
                                     ...
                                     path.smooth = c(0, 99), 
                                     inputdf = df_qx(di = "SDI", q = 1), 
                                     y = y.gender, est_type = "cross_entropy", 
...
hyperpars_phi<- genetic.algorithm(optim.seed = 4567, n = 50, pcrossover = 0.8, 
                                  ...
                                  path.smooth = c(0, 99),
                                  inputdf = df_qx(di = "SDI", q = 1),
                                  y = y.phi, est_type = "cross_entropy",
                                  alpha = 0.5, cv = 2L, no_trees = 100L,
    ...
    for(quant in c(".05", ".25", ".50", ".75", ".95")){
      ...
      for(x.ind in seq_along(x.list)){
        ...
        model.eval(booster = get(model.name), DI = "SDI", sdi = processed.hyp, 
                   Q = 1, eval.var = names(x.list)[x.ind], 
                   eval.seq = x.list[[x.ind]], quant = as.numeric(quant), 
...
for(i in 1:3){
  di <- c("TDI", "HW", "SDI")[i]
  di.var<- c("thoms_discomfort_index", "length_heatwave", "sdi")[i]
  for(quant in c(".05", ".25", ".50", ".75", ".95")){
...
tdi_q2_quantiles<- quantile(full.df_7$thoms_discomfort_index, 
                            probs = c(0.05, 0.95))
model.eval(booster = model_q2_TDI, DI = "TDI", Q = 2, 
           eval.var = "thoms_discomfort_index", 
           eval.seq = seq(tdi_q2_quantiles[1], tdi_q2_quantiles[2], 
                          length.out = 10),
           y.max = 1)

model.eval(booster = model_q2_HW, DI = "HW", Q = 2, 
           eval.var = "length_heatwave", eval.seq = seq(0, 7), y.max = 1)
...
model.eval(booster = model_q2_SDI, DI = "SDI", sdi = sdi_q2, Q = 2, 
           eval.var = "sdi", eval.seq = seq(sdi_q2_quantiles[1], 
                                            sdi_q2_quantiles[2],
                                            length.out = 10), 
           y.max = 1)
...
for(x.ind in seq_along(x.list)){
    model.eval(booster = model_q2_SDI, DI = "SDI", sdi = sdi_q2, Q = 2, 
               eval.var = names(x.list)[x.ind], eval.seq = x.list[[x.ind]], 
               y.max = 1, x.factor.names = x.list.factors[[x.ind]])
}
