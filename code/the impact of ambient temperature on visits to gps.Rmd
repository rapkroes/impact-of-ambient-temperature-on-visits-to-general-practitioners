...
rhs_Thom_q1<- as.data.frame(cbind(full.df_7$thoms_discomfort_index,
...
##tune lambda
tuning.wrapper<- function(lambda_value, alpha_value){
  elastic.net(inputdf = rhs_Thom_q1, y= full.df_7$age, spline.pos = c(1,ncol(rhs_Thom_q1)), spline.knots = c(4,5), sel.loss.function = "quantile", sel.quantile = 0.5, alpha = alpha_value, lambda = lambda_value)
} 
tic()
...
tic()
attempt.p.4.75<- elastic.net(inputdf = rhs_Thom_q1, y= full.df_7$female, spline.pos = 1, spline.knots = 4, sel.loss.function = "proportion", alpha = .1, lambda = .5, tol = 1e-4, max.iter = 75)
toc()
tic()
attempt.p.5.75<- elastic.net(inputdf = rhs_Thom_q1, y= full.df_7$female, spline.pos = 1, spline.knots = 4, sel.loss.function = "proportion", alpha = .1, lambda = .5, tol = 1e-5, max.iter = 75)
toc()
tic()
attempt.p.6.75<- elastic.net(inputdf = rhs_Thom_q1, y= full.df_7$female, spline.pos = 1, spline.knots = 4, sel.loss.function = "proportion", alpha = .1, lambda = .5, tol = 1e-6, max.iter = 75)
toc()

tuning.wrapper(0.5,0.1)
