...
if(fake.data){
  ...
  names.vec<- c("Diag1", "Stamm1", "Konsul1")
...
Diag1<- variable.purger(Diag1, rm.all.PraxisID = TRUE)
Stamm1<- variable.purger(Stamm1, rm.all.PraxisID = FALSE)
Konsul1<- variable.purger(Konsul1, rm.all.PraxisID = TRUE)

#date range: Jan. 1st, 2000 to December 31st, 2023
date.range<- c(730485, 739250)

#location data
location_information<- read.csv("https://github.com/rapkroes/impact-of-ambient-temperature-on-visits-to-general-practitioners/raw/main/other%20data/location%20information.csv", header = TRUE, sep = ";", encoding = "Latin-1")
location.name<- location_information$location.name[!is.na(location_information$praxis_ids)]
...
for(i in seq_along(location.name)){
  ...
  weatherdata.transformation(im, sel.quantile = .90, loc = location.name[i],
                             dr = date.range)
...
workspace.save(TRUE)
```
We must check Diag's icd10 column: Since the icd10 field is a text field in the GP's software, the data might contain typing errors or atypical notation of diseases, which survived pre-processing. Several patterns are tested. ICD-10 codes to not meet either of the patterns are discarded.
```{r}
sus.entry.vec<- suspicious.Diag.entries(icd10vec = Diag1$icd10, no.workers = 10)
diag_sus<- Diag1
diag_sus$class<- icd10.to.class(diag_sus$icd10, no.workers = 10)
View(diag_sus[sus.entry.vec,])
diag_rm.sus<- Diag1[(!sus.entry.vec) & !is.na(diag_sus$class),]
```
...
chronic.sel<- grepl("DD",diag_rm.sus$Typ)
chronic.df<- data.frame(uniPatID = diag_rm.sus$uniPatID[chronic.sel], 
                        diag_category = diag_rm.sus$icd10[chronic.sel])|>
  distinct()
chronic.df$diag_category<- icd10.to.class(chronic.df$diag_category, 
                                          no.workers = global.workers)
im<- diag_rm.sus[!chronic.sel,]|>
  filter(TG_DateNum >= date.range[1] & TG_DateNum <= date.range[2])|>
  distinct()
full.df_1<- add.chronic(im, chronic.df)
full.df_1$diag_class<- icd10.to.class(full.df_1$icd10, 
                                      no.workers = global.workers)
full.df_1.1<- add.last.visit(full.df_1, no.splits = 50, 
                             no.workers = global.workers)
workspace.save(make.saves)

### stamm data
full.df_1.2<- add.stamm.new.par(full.df_1.1, Stamm1, no.splits = 50, 
                                no.workers =global.workers)
# gender variable
...
### konsul
full.df_2<- add.konsul(full.df_1.3, Konsul1, no.splits = 50, 
                       no.workers =global.workers)
...
full.df_4.2<- add.covid(full.df_4,covid.data, no.workers =global.workers)
full.df_4.2$covid_positive_incidence<- full.df_4.2$covid_7_day_incidence > 0
workspace.save(make.saves)

### risk factors
full.df_5<- full.df_4.2
full.df_5$smoking<- risk.factor.merger(full.df_5$A_Rauchen,
                                       full.df_5$RF_Rauchen)
full.df_5$alcohol<- risk.factor.merger(full.df_5$A_Alkohol,
                                       full.df_5$RF_Alkohol)
full.df_5$sport<- risk.factor.merger(full.df_5$A_Sport,
                                     full.df_5$RF_Sport)
workspace.save(make.saves)

### add weather variables
full.df_6<- full.df_5[!is.na(full.df_5$PraxisID),] #for all entries except 5 we have Stamm1 data, we remove them in this step.
full.df_6<- add.weather(full.df_6, no.workers = global.workers)
workspace.save(make.saves)

###add daylight data
full.df_7<- full.df_6
full.df_7<- add.daylight(full.df_6, no.workers = global.workers)|>
  arrange(TG_DateNum)

###repair data after viewing
full.df_7$Geburtsjahr[full.df_7$Geburtsjahr == 0]<- NA
full.df_7$age<- TG_DateNum2year(full.df_7$TG_DateNum) - full.df_7$Geburtsjahr
# remove 2780 observations of people aged 100 and above, and 103 observations of ages less than 0.
full.df_7<- filter(full.df_7, age <= 100, age >= 0) 

if(make.saves){
  workspace.save(make.saves)
  write.table(full.df_7, file = file.path("N:/StudentischeHilfskraefte/_Kroes (Christoph)/Routinedaten/03_Hitze/R/final_df.csv"), row.names = FALSE)
}

```
```{r}
#Workspace clearing: remove 
rm.list_1<- ls()[grepl("daylight_", ls()) | grepl("transformed_weather_", ls())]
rm.list_2<- ls()[grepl("Diag", ls()) | grepl("Stamm", ls()) | 
                   grepl("Konsul", ls())]
rm.list_3<- ls()[grepl("full.df_", ls())]
rm.list_3<- rm.list_3[rm.list_3 != "full.df_7"]
rm(list = c(rm.list_1, rm.list_2, rm.list_3))
workspace.save(TRUE)
```
...
write.table(table_1, file = file.path("N:/StudentischeHilfskraefte/_Kroes (Christoph)/Routinedaten/03_Hitze/R/table_1.csv"), sep = "; ", row.names = FALSE)
...
hyperpars_age<- genetic.algorithm(optim.seed = 4567, n = 50, pcrossover = 0.8, 
                                  pmutation = 0.1, maxiter = it, elitism = 4, 
                                  digits = dg, sdi = TRUE, lr = c(0,1), 
                                  ...
                                  no_threads = global.workers, 
                                  early_stopping = 10L)
workspace.save(make.saves)

hyperpars_gender<- genetic.algorithm(optim.seed = 4567, n = 50, 
                                     pcrossover = 0.8, pmutation = 0.1, 
                                     maxiter = it, elitism = 4, digits = dg, 
                                     sdi = TRUE, lr = c(0, 1), 
                                     no.leaves = c(2, 40), max.depth = c(1, 10), 
                                     min.data.in.leaf = c(2, 1e7), 
                                     feature.fraction = c(0, 1), 
                                     cat.l2 = c(0, 99), extra.trees = c(0, 1), 
                                     top.rate = c(0, 0.5), 
                                     other.rate = c(0, 0.5), 
                                     cat.smooth = c(0, 99), 
                                     path.smooth = c(0, 99), 
...
hyperpars_phi<- genetic.algorithm(optim.seed = 4567, n = 50, pcrossover = 0.8, 
                                  pmutation = 0.1, maxiter = it, elitism = 4, 
                                  digits = dg, sdi = TRUE, lr = c(0, 1), 
                                  no.leaves = c(2, 40), max.depth = c(1, 10),
                                  min.data.in.leaf = c(2, 1e7), 
                                  ...
                                  alpha = 0.5, cv = 2L, no_trees = 100L,
                                  no_threads = global.workers, early_stopping = 10)
workspace.save(make.saves)
    ...
    if(model.list_1[[model.ind]] == "quantile"){
      ...
      for(quant in c(".05", ".25", ".50", ".75", ".95")){
        ...
        assign(model.name, ga2model(hyp, inputdf = df_qx(di = di, q = 1),
                                    ...
                                    no.trees = 100L, no.threads = global.workers
                                    ))
      }
    }else{
...
for(model.ind in seq_along(model.list_1)){
  ...
  if(model.list_1[[model.ind]] == "quantile"){
    for(quant in c(".05", ".25", ".50", ".75", ".95")){
      y.range<- ifelse(model.ind == 1, 100, 30)
      for(x.ind in seq_along(x.list)){
        model.name<- paste0("model_", names(model.list_1)[model.ind], quant, 
                            "_SDI")
            model.eval(booster = get(model.name), DI = "SDI", sdi = processed.hyp,
    ...
    for(x.ind in seq_along(x.list)){
      ...
      model.eval(booster = get(model.name), DI = "SDI", sdi = processed.hyp,
                 Q = 1, no.draws = 100, eval.var = names(x.list)[x.ind],
                 eval.seq = x.list[[x.ind]], seed = 8181818, y.max = 1, 
                 y.name = names(model.list_1)[model.ind], 
                 x.factor.names = x.list.factors[[x.ind]])
    }
  }
} #model.name??? age quantiles??? 
#performance plots
practice.ids<- sort(unique(full.df_7$PraxisID))
for(i in practice.ids){
  performance.plots("age", di = "SDI", practiceID = i, galist = hyperpars_age,
                    y.name = "age", y.range = c(0, 100))
  performance.plots("gender", di = "SDI", practiceID = i, 
                    galist = hyperpars_gender, y.name = "proportion female", 
                    y.range = c(0, 1))
  performance.plots("phi", di = "SDI", practiceID = i, galist = hyperpars_phi,
                    y.name = "proportion privately insured", y.range = c(0, 1))
  performance.plots("chronic", di = "SDI", practiceID = i, hyperpars_chronic,
                    y.name = "number of chronic diseases", y.range = c(0, 30))
}
workspace.save(make.saves)
...
hyperpars_q2<- genetic.algorithm(optim.seed = 4567, n = 50, pcrossover = 0.8, 
                                 ...
                                 y = train_diag_class, 
                                 est_type = "multiclass", cv = 3L, 
                                 no_trees = 100L, no_threads = global.workers, 
                                 early_stopping = 10L)
workspace.save(make.saves)

# additional tuning
lr.vec<- 10^(seq(0, -3))
hyp.tun<- hyperpars_q2
add.tun.results<- list()
for(i in seq_along(lr.vec)){
  hyp.tun$best_in_class[1]<- lr.vec[i]
  add.tun.results[[i]]<- ga2performance.eval(hyp.tun, 
                                             inputdf = df_qx(di = "SDI", q = 2),
                                             y = train_diag_class, 
                                             est.type = "multiclass", cv = 4)
}

model.eval_new(model_q2_SDI, DI = "SDI", sdi = sdi_q2, Q = 2, eval.var = "sdi",
               eval.seq = seq(6353215, 8030782, length.out = 100), 
               downsample = 0.05, seed = 6181, y.max = 1, y.name = "SDI")

set.seed(616164)
model_q2_short<- ga2model(hyperpars_q2, inputdf = df_qx(di = "SDI", q = 2)[sample(seq(1, nrow(full.df_7)))],
                     y = train_diag_class, est.type = "multiclass",
                     no.trees = 100L, no.threads = global.workers)
oos.ndf<- df_qx(di = "SDI", q = 2)
oos.ndf<- oos.ndf[, !(grepl("temperature", colnames(oos.ndf)) | grepl("humidity", colnames(oos.ndf)))]
oos.ndf$sdi<- full.df_7$sdi_q2
oos.ndf<- data.matrix(oos.ndf)
oos<- matrix(predict(object = model_q2_SDI, newdata = oos.ndf, type = "response"), ncol = 11)
oos.mode.pred<- round(oos) %*% seq(1, 11)
View(cbind(oos.mode.pred, full.df_7$diag_class))

ndf<- df_qx(di = "TDI", q = 2)
model_q2_short<- ga2model(hyperpars_q2, inputdf = ndf,
                     y = train_diag_class, est.type = "multiclass",
                     no.trees = 100L, no.threads = global.workers)
oos.ndf<- df_qx(di = "TDI", q = 2)|>
  data.matrix()
oos.ndf<- data.matrix(oos.ndf)
oos<- matrix(predict(object = model_q2_SDI, newdata = oos.ndf, type = "response"), ncol = 11)
oos.mode.pred<- round(oos) %*% seq(1, 11)
...
model_q2_TDI<- ga2model(hyperpars_q2, inputdf = df_qx(di = "TDI", q = 2),
                     y = train_diag_class, est.type = "multiclass",
                     no.trees = 100L, no.threads = global.workers)
model_q2_HW<- ga2model(hyperpars_q2, inputdf = df_qx(di = "HW", q = 2),
                     y = train_diag_class, est.type = "multiclass",
                     no.trees = 100L, no.threads = global.workers)
model_q2_SDI<- ga2model(hyperpars_q2, inputdf = df_qx(di = "SDI", q = 2),
                     y = train_diag_class, est.type = "multiclass",
                     no.trees = 100L, no.threads = global.workers)
View(df_qx(di = "SDI", q = 2))
View(df_qx(di = "TDI", q = 2))
...
sdi_q2<- unlist(hyperpars_q2$best_in_class[,12:21])
...
sdi_q2_quantiles<- quantile(full.df_7$sdi_q2, probs = c(0.05, 0.95))
model.eval(booster = model_q2_SDI, DI = "SDI", sdi = sdi_q2, Q = 2, 
           no.draws = 100, eval.var = "sdi", 
           eval.seq = seq(3504659, 5182794, length.out = 500), 
           seed = sel.seed, y.max = 1)
# eval.seq corresponds to -7 to 30 degrees celsius
